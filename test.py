...
def is_validation(user_data):
    ...
    if len(user_data['name']) < 2 or 30 < len(user_data['name']):
...
def create_user(user_check):
    ...
    if user_check == 'blocked':
        ...
        wrong_data += 1
        user_list.append(user)
        for info in user_check[1]:
            user_list[-1][info] = None
    

for user in user_data:
    create_user(is_validation(user))
print(f'잘못된 데이터로 구성된 유저의 수는 {wrong_data} 입니다.')
print(user_data)